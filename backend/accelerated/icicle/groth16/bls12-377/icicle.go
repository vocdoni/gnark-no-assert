...
func Prove(r1cs *cs.R1CS, pk *ProvingKey, fullWitness witness.Witness, cfg *icicle.Config) (*groth16_bls12377.Proof, error) {
	...
	solverOpts = append(solverOpts, solver.OverrideHint(bsb22ID, func(_ *big.Int, in []*big.Int, out []*big.Int) error {
		...
		<-ckBasisMsmDone
		runtime.KeepAlive(privateCommittedValues[i]) // host slice must outlive async device copy
	...
	if numCommitmentKeys > 0 {
		...
		for i := range privateCommittedValues {
			runtime.KeepAlive(privateCommittedValues[i]) // keep host buffers alive across chunked MSM chunks
		}
		if isProfileMode {
...
func computeH(a, b, c []fr.Element, pk *ProvingKey, device *icicle_runtime.Device) icicle_core.DeviceSlice {
	...
	computeInttNttOnDevice := func(args ...any) {
		...
		runtime.KeepAlive(scalars) // guard host slice while CUDA stream completes
		close(channel)
	}
